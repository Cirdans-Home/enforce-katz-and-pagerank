function [Delta,varargout] = enforce_pagerank_hp(A,alpha,pihat,v,P,beta,tol)
...
if beta == 1
    ...
    H      = mp(2)*mp(speye(reduced_size));
    work = spdiags(mp(1)./mp(deg),0,n,n)*pihat;
    L       = [kron(work.',mp(speye(n)))*(K*proj.');...
        kron(mp(ones(n,1).'),mp(speye(n)))*proj.'];
    g       = reshape(A,n*n,1);
    g(free_variables) = mp(0);
    g        = proj*g;

    b       = [   (mp(1)/alpha).*(pihat -(mp(1)-alpha).*v) - A.'*(spdiags(mp(1)./deg,0,n,n)*pihat)+kron(work.',mp(speye(n)))*(K*(proj.'*g) )  ;...
        kron(mp(ones(n,1)).',mp(speye(n)))*(proj.'*g) ];
    ...
    if nargout >= 3
        I              = mp(speye(n,n));
        rhat         = min(diag( spdiags(mp(1)./deg,0,n,n)*(A+Delta) ));

        if rhat < 1e-8
            rhat          = mp(1)- alpha*rhat;
            r               = rhat;
            alphahat   = mp(1)- (mp(1)-alpha)/r;
            Phat         = 1/(r-mp(1)+alpha).*(alpha*(spdiags(mp(1)./deg,0,n,n)*(A+Delta))+(r-mp(1)).*mp(speye(n)) );
            varargout{2}     =  (I - alphahat*Phat.')\((1-alphahat).*v) ;
        else
            rhat      = 0;
            varargout{2} =  (I - alpha*(spdiags(mp(1)./deg,0,n,n)*(A+Delta)).')\((mp(1)-alpha).*v) ;
        end
        if nargout >= 4
            varargout{3} = rhat;
        end
    ...
    tau = (mp(1)-beta)/beta;
    Q   = mp(2)*mp(speye(reduced_size));
    work = spdiags(mp(1)./deg,0,n,n)*pihat;
    L    = [kron(work.',mp(speye(n)))*(K*proj.');...
        kron(mp(ones(n,1)).',mp(speye(n)))*proj.'];


    c       = reshape(A,n*n,1);
    c(free_variables) = mp(0);
    c        = proj*c;

    b       = [   (mp(1)/alpha).*(pihat -(mp(1)-alpha).*v) - A.'*(spdiags(mp(1)./deg,0,n,n)*pihat)+kron(work.',mp(speye(n)))*(K*(proj.'*c) )  ;...
        kron(mp(ones(n,1)).',mp(speye(n)))*(proj.'*c) ];
    b        = [b;-c(constr_var)];
    H       = blkdiag(Q,mp(sparse(length(constr_var),length(constr_var))),mp(sparse(length(constr_var),length(constr_var))));
    g        = [-mp(2).*c; tau.*mp(ones(length(constr_var),1)); tau.*mp(ones(length(constr_var),1))];
    II        = mp(speye(reduced_size));
    L        = [L, mp(sparse(2*n,length(constr_var))), mp(sparse(2*n,length(constr_var)));...
        - II(constr_var,:),     mp(speye(length(constr_var))),      -mp(speye(length(constr_var)))];
    ...
    if nargout >= 3
        ...
        if rhat < mp(1e-8)
            ...
            r               = rhat;
            alphahat   = mp(1)- (mp(1)-alpha)/r;
            Phat         = mp(1)/(r-mp(1)+alpha).*(alpha*(spdiags(mp(1)./deg,0,n,n)*(A+Delta))+(r-mp(1)).*mp(speye(n)));
            varargout{2}     =  (I - alphahat*Phat.')\((mp(1)-alphahat).*v) ;
        else
            rhat      = 0;
            varargout{2} =  (I - alpha*(spdiags(mp(1)./deg,0,n,n)*(A+Delta)).')\((mp(1)-alpha).*v) ;
        end
...
K = mp(speye(n*n)); % Initialize an identity matrix
...
if (nargin < 6 || isempty(tol));            tol = mp(1e-4);      end
...
e_pos_vars = mp(ones(num_of_pos_vars,1));
...
D = sum(A.^2,2) + mp(10);
Jacobi_Prec = @(x) (1./D).*x;
NE_fun = @(x) (A*(A_tr*x) + mp(10).*x);
[x,~] = pcg(NE_fun,b,mp(10)*tol,min(1000,m),Jacobi_Prec);
x = A_tr*x;
[y,~] = pcg(NE_fun,A*(c+Q*x),mp(10)*tol,min(1000,m),Jacobi_Prec);
z = c+ Q*x - A_tr*y;
% ======================================================================= %
if (norm(x(pos_vars)) <= mp(1e-4))
    x(pos_vars) = mp(0.1).*mp(ones(num_of_pos_vars,1)); % 0.1 is chosen arbitrarily
end

if (norm(z(pos_vars)) <= mp(1e-4))
    z(pos_vars) = mp(0.1).*mp(ones(num_of_pos_vars,1)); % 0.1 is chosen arbitrarily
end

delta_x = max(-mp(1.5)*min(x(pos_vars)),mp(0));
delta_z = max(-mp(1.5)*min(z(pos_vars)),mp(0));
temp_product = (x(pos_vars) + (delta_x.*e_pos_vars))'*(z(pos_vars) ...
    + (delta_z.*e_pos_vars));
delta_x_bar = delta_x + (mp(0.5)*temp_product)/(sum(z(pos_vars),1) + ...
    mp(num_of_pos_vars)*delta_z);
delta_z_bar = delta_z + (mp(0.5)*temp_product)/(sum(x(pos_vars),1) + ...
    mp(num_of_pos_vars)*delta_x);
...
while (iter < maxit)
    ...
    if  (norm(nr_res_p)/(max(mp(10),norm(b))) < mp(tol) && norm(nr_res_d)/(max(mp(10),norm(c))) < mp(tol)*mp(1e3) &&  mu < mp(tol) )
        if printlevel > 0
...
k     = find(P);
N    = size(P,1);
proj = sparse(1:1:length(k),k,mp(ones(length(k),1)), length(k),N*N );
end
...
if num_of_pos_vars > 0
    mu = (x(pos_vars)'*z(pos_vars))/mp(num_of_pos_vars);
else
    mu = mp(0);
end
end
...
y=yk;
z=zk;
e_pos_vars = mp(ones(num_of_pos_vars,1)); % Vector of ones of dimension |C|.
% ==================================================================================================================== %
% Initialize parameters
% -------------------------------------------------------------------------------------------------------------------- %
iter = 0;
alpha_x = mp(0);     % Step-length for primal variables (initialization)
alpha_z = mp(0);     % Step-length for dual variables (initialization)
sigmamin = mp(0.05); % Heuristic value.
sigmamax = mp(0.95); % Heuristic value.
sigma = mp(0);
OInfo.opt = 0;

if (num_of_pos_vars > 0)                             % Defined only when non-negativity constraints are present.
    mu = (x(pos_vars)'*z(pos_vars))/mp(num_of_pos_vars); % Initial value of mu.
    res_mu = mp(zeros(n,1));
else
...
while (iter < maxit)
    ...
    % ---------------------------------------------------------------------------------------------------------------- %
    if (res_n < mp(1e4)*tol*min(mp(1),norm(x-xk)+norm(y-yk)) || norm(x-xk) <= eps(x(1)) || norm(y-yk) <= eps(x(1))) && iter > 1 
        OInfo.IPMIter= iter;
        OInfo.opt    = 1;
        OInfo.nnz    = NS.nnz;
        break;
    ...
    switch pc
        case 1 % No predictor-corrector.
            ...
            % ------------------------------------------------------------------------------------------------------------ %
            if (iter > 1)
                sigma = max(mp(1)-alpha_x,mp(1)-alpha_z)^5;
            ...
            % ============================================================================================================ %
            if (num_of_pos_vars > 0)
                res_mu(pos_vars) = (sigma*mu).*mp(e_pos_vars) - x(pos_vars).*z(pos_vars);
            end
            % ============================================================================================================ %
        ...
        case 2 % Mehrotra predictor-corrector. ONLY when num_of_pos_vars > 0!!
            ...
            if (instability == true) % Checking if the matrix is too ill-conditioned. Mitigate it.
                if (retry_p < max_tries)
                    ...
                    iter = iter -1;
                    retry_p = retry_p + 1;
                    reg_limit = min(mp(reg_limit)*mp(100),mp(tol));
                    continue;
                else
                    fprintf('The system matrix is too ill-conditioned.\n');
                    OInfo.opt = 2;
            ...
            idx = false(n,1);
            idz = false(n,1);
            idx(pos_vars) = dx(pos_vars) < mp(0); % Select all the negative dx's (dz's respectively)
            idz(pos_vars) = dz(pos_vars) < mp(0);
            alphamax_x = min([mp(1);-x(idx)./dx(idx)]);
            alphamax_z = min([mp(1);-z(idz)./dz(idz)]);
            tau = mp(0.995);
            ...
            % ---------------------------------------------------------------------------------------------------------------- %
            res_mu(pos_vars) = mu.*mp(e_pos_vars) - dx(pos_vars).*dz(pos_vars);
            % ============================================================================================================ %
            % Solve the Newton system with the predictor right hand side -> Optimistic view, solve as if you wanted to
            %                                                               solve the original problem in 1 iteration.
            % ------------------------------------------------------------------------------------------------------------ %
            [dx_c,dy_c,dz_c,instability] = Newton_backsolve(NS,mp(zeros(m,1)),mp(zeros(n,1)),res_mu,pos_vars,free_variables,IterStruct );
            if (instability == true) % Checking if the matrix is too ill-conditioned. Mitigate it.
                if (retry_c < max_tries)
                    ...
                    retry_c = retry_c + 1;
                    mu = mu_prev;
                    reg_limit = min(mp(reg_limit)*mp(100),tol);
                    continue;
                else
    ...
    if (num_of_pos_vars > 0)
        ...
        alpha_x = mp(1);         % If we have no inequality constraints, Newton method is exact -> Take full step.
        alpha_z = mp(1);
    end
    % ================================================================================================================ %
    ...
    if (num_of_pos_vars > mp(0)) % Only if we have non-negativity constraints.
        mu_prev = mu;
...
r{1}(pos_var) = x(pos_var)-max(mp(0),x(pos_var)-r{1}(pos_var));
...
if strcmp(Struct.Fact,'ldl')
    ...
    K       = A*(NS.D\A_tr)+spdiags(delta.*mp(ones(m,1)),0,m,m);
    [NS.L,~] = chol(full(K));
    NS.pp = ((1:size(NS.L,1))');
    NS.nnz   =  nnz(NS.L);
end
    ...
    for j = 1:size(A,2)
        ...
        if (any(rows))
            ...
            if (maximum*minimum > 10^(-12) && maximum*minimum < 10^(12))
                D(j) = mp(1)/sqrt(maximum*minimum);
            else
                D(j) = mp(1);
            end
        else
            D(j) = mp(1); % Extreme case, where one column is all zeros.
        end
    end
...
elseif (scale_option == 4)
    ...
    for j = 1:size(A,2)
        ...
        if (any(rows))
            ...
            maximum = max(pos_A(rows,j));
            minimum = min(pos_A(rows,j));
            if (maximum > mp(10^3) && minimum < mp(10^(-3)))
                p = nextpow2(sqrt(maximum*minimum));
                D(j) = 1/(2^(p-1));
            elseif (mp(1)/minimum > maximum && minimum > mp(10^(-6)))
                p = nextpow2(minimum);
                D(j) = mp(1)/(mp(2)^(p-1));
            elseif (maximum < 10^(6))
                p = nextpow2(maximum);
