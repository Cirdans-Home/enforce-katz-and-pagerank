function [Delta,varargout] = enforce_katz_hp(A,alpha,muhat,P,beta,tol)
...
if beta == 1
    ...
    L = kron(muhat.',mp(speye(n)))*proj.';
    b = (mp('1')/alpha).*(muhat-mp('1'))-A*muhat+L*g;
    ...
    L   = kron(muhat.',mp(speye(n)))*proj.';
    b   = [(mp(1)/alpha).*(muhat-mp(1))-A*muhat+L*c;-c];
    H   = blkdiag(Q,mp(sparse(reduced_size,reduced_size)),mp(sparse(reduced_size,reduced_size)));
    g   = [-mp(2).*c; tau.*mp(ones(reduced_size,1)); tau.*mp(ones(reduced_size,1))];
    L   = [L, mp(sparse(n,reduced_size)), mp(sparse(n,reduced_size));...
        - mp(speye(reduced_size)), mp(speye(reduced_size)),      -mp(speye(reduced_size))];
    % Running the solver
    free_variables = [];
    IterStruct     = struct();
    IterStruct.Fact                   = 'chol';
    rho               = mp('1e-9');%mp('1e-6')*tol;
    delta          = rho;
    pc_mode        = 2;
    tic;
